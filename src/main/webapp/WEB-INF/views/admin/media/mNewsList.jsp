		...
		<div id="container" class="ly_container"> 
			...
			<main id="contents" class="ly_contents">
				...
				<div class="board_search">
					<form action="mNewsList" class="search_group" autocomplete="off" id="searchForm">
				...
				<div class="page_header mb10 mt20"> 
					<div class="types">
						<a href="mNewsList?page=${currentPage}&keyWord=${keyWord}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}&type=card" class="el_btn type card"><span class="blind">카드 타입</span></a>
						<a href="mNewsList?page=${currentPage}&keyWord=${keyWord}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}&type=list" class="el_btn type list on"><span class="blind">리스트 타입</span></a>
					</div>
				</div> 
				<!-- board list -->
				<table class="tbl type2 mt30">
					...
					<tbody>
						...
						<tr>
							<td>${newsList.rownum}</td>
							<td style="text-align: left;"><a href="mNewsWrite?num=${newsList.num }">${newsList.subject}</a></td>
							<td>${newsList.wdateformat}</td>
							<td>${newsList.etc1}</td>
						</tr>
					</c:forEach>
