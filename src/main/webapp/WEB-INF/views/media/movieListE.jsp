			...
			<section class="section">
				<div class="ly_inner md">
					...
					<article class="search_box type2 mb0">
						<form action="movieListE" class="search_box_form grid" id="searchForm">
						...
						<div class="row row1">
								...
								<a href="movieListE" class="el_btn refresh p_hide" aria-label="새로고침" ><span class="p_hide">새로고침</span></a>
							</div>
							<div class="row row2">
								<div class="col grow">
								...
								<div class="col m_flex">
									...
									<a href="movieListE" class="el_btn refresh p_show" aria-label="새로고침"></a>
								</div>
							</div>
							<div class="tags bl_tag_list">
								<c:forEach items="${searchKeywordList}" var="searchKeywordList">
									<a href="movieListE?keyWord=${searchKeywordList.search_keyword}" class="tag"><span>#${searchKeywordList.search_keyword}</span></a>
								</c:forEach>
							</div>
						</form>
					...
					<c:if test="${topMovie != null }">
					<!-- 가장 최근 영상 -->
					<article class="board_latest_video mt40-30">
						<div class="video el_video">
							<iframe width="560" height="315" src="https://www.youtube.com/embed/${topMovie.linkurl }" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
						</div>
						<div class="content">
							<p class="tit txt_line2">${topMovie.subject}</p>
							<c:if test="${topMovieSchedule != null }">
							<p class="date">${topMovieSchedule.game_date_format}.(${topMovieSchedule.week_day }) ${topMovieSchedule.game_start_format } ${topMovieSchedule.stadium_name_1 }</p>
							</c:if>
						</div>
					</article>
					<!-- //가장 최근 영상 -->
					...
					<article class="mt50">
						...
						<div class="board_header">
							<div class="bbs_cate_list">
								<a href="movieListE?keyWord=${keyWord}&season=${season}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}&otype=${otype}" class="cate <c:if test="${wtype =='all'}">active</c:if>">전체</a>
								<a href="movieListE?keyWord=${keyWord}&season=${season}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}&wtype=U&otype=${otype}" class="cate <c:if test="${wtype =='U'}">active</c:if>">영상</a>
								<a href="movieListE?keyWord=${keyWord}&season=${season}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}&wtype=S&otype=${otype}" class="cate <c:if test="${wtype =='S'}">active</c:if>">숏츠</a>
							</div>

							<div class="bbs_sort_list">
								<a href="movieListE?keyWord=${keyWord}&season=${season}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}&wtype=${wtype}&otype=num" class="sort <c:if test="${otype =='num'}">active</c:if>">최신순</a>
								<a href="movieListE?keyWord=${keyWord}&season=${season}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}&wtype=${wtype}&otype=visited" class="sort <c:if test="${otype =='visited'}">active</c:if>">조회순</a>
							</div>
						</div>
						<!-- //board header -->
						...
						<c:forEach items="${movieList}" var="movieList">
							<li class="item">
								<a href="movieListEDetail?num=${movieList.num}&listpage=${currentPage}&keyWord=${keyWord}&season=${season}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}&wtype=${wtype}&otype=${otype}" class="bbs_box">
								<c:choose>
									...
									<p class="bbs_info_wrap">
										<span class="bbs_info date">${movieList.wdateformat}</span>
					...
					<a href="movieListE?page=1&keyWord=${keyWord}&season=${season}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}&wtype=${wtype}&otype=${otype}" class="page_link ico first"><span class="blind">처음</span></a> <!-- 비활성화시 disabled 추가 -->
					</c:if>
					
					<!-- 이전 블럭으로 이동 -->
					<c:if test="${startPage2 gt 1 }">
                       	<a href="movieListE?page=${startPage2-1}&keyWord=${keyWord}&season=${season}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}&wtype=${wtype}&otype=${otype}" class="page_link ico prev"><span class="blind">이전</span></a> <!-- 비활성화시 disabled 추가 -->
                    </c:if>
                    ...
                    <c:forEach var="p" begin="${startPage2}" end="${endPage2}" step="1">
                      ...
                      <c:if test="${p ne currentPage }">
                      	<c:url var="movieListE" value="movieListE?keyWord=${keyWord}&season=${season}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}&wtype=${wtype}&otype=${otype}">
	 					<c:param name="page" value="${p}" />
	 					</c:url>
	 					<a href="${movieListE}" class="page_link">${p}</a>
                      </c:if>
                    </c:forEach>
                    
                    <!-- 다음 블럭으로 이동 -->
                    <c:if test="${endPage2 ne maxPage2 && maxPage2 > 1}">
					<a href="movieListE?page=${endPage2+1}&keyWord=${keyWord}&season=${season}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}&wtype=${wtype}&otype=${otype}" class="page_link ico next"><span class="blind">다음</span></a>
                    </c:if>
                    ...
                    <c:if test="${maxPage2 > 0}">
					<a href="movieListE?page=${maxPage2}&keyWord=${keyWord}&season=${season}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}&wtype=${wtype}&otype=${otype}" class="page_link ico last"><span class="blind">마지막</span></a>
					</c:if>
				</div>
				<!-- // pagination -->
						<!-- pagination -->
					...
					<c:if test="${startPage gt 1 }">
                       	<a href="movieListE?page=${startPage-1}&keyWord=${keyWord}&season=${season}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}&wtype=${wtype}&otype=${otype}" class="page_link ico prev"><span class="blind">이전</span></a> <!-- 비활성화시 disabled 추가 -->
                    </c:if>
					...
					<a href="movieListE?page=${endPage+1}&keyWord=${keyWord}&season=${season}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}&wtype=${wtype}&otype=${otype}" class="page_link ico next"><span class="blind">다음</span></a>
                    </c:if>
                    ...
                    <c:if test="${maxPage > 0}">
					<a href="movieListE?page=${maxPage}&keyWord=${keyWord}&season=${season}&sdate=${sdate}&edate=${edate}&round=${round}&game=${game}&player=${player}&wtype=${wtype}&otype=${otype}" class="page_link ico last"><span class="blind">마지막</span></a>
					</c:if>
				</div>
				<!-- // pagination -->
						</c:if>
